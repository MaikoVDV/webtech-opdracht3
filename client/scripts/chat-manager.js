...
window.addEventListener("load", async () => {
  ...
  inputForm.addEventListener("submit", async (event) => {
    ...
    const postMsgQuery = await fetch(`/api/send-chat/${currentOpenFriend.id}`, {
      method: "POST",
...
async function openChat(friendId) {
  ...
  if (friendQuery.ok) {
    try {
      ...
      chatHeader.textContent = `${friendData.first_name} ${friendData.last_name}`;
    } catch (ex) {
  ...
  messages.forEach(message => {
    ...
    const timeString = `${msgDate.getHours()}:${msgDate.getMinutes()}`
